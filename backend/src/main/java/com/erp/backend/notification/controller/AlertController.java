...
public class AlertController {
    ...
    @PutMapping("/{alertId}")
    public ResponseEntity<ApiResponse<Void>> readAlert(@PathVariable int alertId, @RequestParam int loginId){
        alertService.readMessage(alertId,loginId);
        return ResponseEntity.ok(ApiResponse.success("알람 읽음처리",null));
    }

    @GetMapping
    public ResponseEntity<ApiResponse<List<AlertVO>>> getAllLis(@RequestParam int loginId){
        List<AlertVO> list = alertService.getUserAlertList(loginId);
