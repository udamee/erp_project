...
public interface SalesOrderMapper {
    ...
    //상태 조건에 따른 주문 목록 조회
    List<SalesOrderVO> findAllSalesOrders(String status,int offset,int size);
    //상품에 따른 전체 로트 정보 조회
    ...
    //페이징용 상태별 총 수 계산
    int findCountsForSalesOrders(String status);
    //상태 필터링 후 총 개수
    List<SalesOrderStatusCountDTO> findCountsByStatus();

    List<ProductVO> findAllActiveProducts();

    List<SalesOrderVO> findAllCustomers();
}
